...
class MailController extends Controller
{
    public function send_coupon($coupon_time,$coupon_condition,$coupon_number,$coupon_code){
        ...
        Toastr::success('Gửi mã khuyến mãi khách thường thành công','Thông báo');
	...
	public function send_coupon_vip($coupon_time,$coupon_condition,$coupon_number,$coupon_code){
        ...
        Toastr::success('Gửi mã khuyến mãi khách vip thành công','Thông báo');
		return redirect()->back();
    }
    public function recover_pass(Request $request){
    	$data = $request->all();
		$now = Carbon::now('Asia/Ho_Chi_Minh')->format('d-m-Y'); //hiện thị ngày
		$title_mail = "Lấy Lại Mật Khẩu Từ K-Shopper".' '.$now;
		...
		if($customer){
            ...
            if($count_customer==0){
                return redirect()->back()->with('error', 'Email chưa được đăng ký để khôi phục mật khẩu');
               	...
               	$token_random = Str::random(); //tạo token trong laravel
    ...
    public function reset_new_pass(Request $request){
        ...
        if($count>0){
                ...
                if( $reset->customer_password == md5($data['password_account']))
                {
                    return redirect()->back()->with('error', 'Vui Lòng Điền Mật Khẩu Mới Khác Với Mật Khẩu Đang Sử Dụng');
                }if(md5($data['password_account']) != md5($data['password_account2'])){
                    return redirect()->back()->with('error', 'Mật Khẩu Đã Nhập Không Trùng Nhau Vui Lòng Kiểm Tra Lại');
                }
    ...
    public function update_new_pass(Request $request){
        ...
        $cate_product = Category_product::where('category_status','0')->orderby('category_parent','desc')->orderby('category_order','ASC')->get();

        $brand_product =Brand::where('brand_status','0')->orderby('brand_id','desc')->get();


        $all_product =Product::where('product_status','0')->orderByRaw('RAND()')->paginate(6);

        $cate_pro_tabs = Category_product::where('category_parent','<>',0)->orderBy('category_order','ASC')->get();
    ...
    public function quen_mat_khau(Request $request){
        ...
        $all_product =Product::where('product_status','0')->orderby('product_id','ASC')->paginate(6);
